...
class Dis_content(nn.Module):
    def __init__(self):
        ...
        model = []
        model += [LeakyReLUConv2d(256, 256, kernel_size=3, stride=1, padding=1, norm='None')]  # Instance
        model += [LeakyReLUConv2d(256, 256, kernel_size=3, stride=1, padding=1, norm='None')]
        model += [LeakyReLUConv2d(256, 256, kernel_size=3, stride=1, padding=1, norm='None')]
        model += [LeakyReLUConv2d(256, 256, kernel_size=3, stride=1, padding=1, norm='None')]
...
class AdaINGen(nn.Module):
    ...
    def __init__(self, input_dim, params):
        ...
        # style encoder
        self.enc_noise = NoiseEncoder(4, input_dim, dim, style_dim, norm='none', activ=activ, pad_type=pad_type)
...
class VAEGen(nn.Module):
    ...
    def __init__(self, input_dim, params):
        ...
        self.noise = NoiseEncoder(n_downsample, input_dim, dim, self.enc.output_dim, 'bn', activ,
    ...
    def encode_noise(self, images):
        noisehiddens = self.noise(images)
        return noisehiddens
...
class Transpose2dBlock(nn.Module):
    def __init__(self, input_dim, output_dim, kernel_size, stride,
        ...
        self.use_bias = True
        # initialize convolution
        self.conv = nn.ConvTranspose2d(input_dim, output_dim, kernel_size, stride, padding=1, bias=self.use_bias, output_padding=1)
        # initialize padding
        if pad_type == 'reflect':
            self.pad = nn.ReflectionPad2d(padding)
        elif pad_type == 'replicate':
